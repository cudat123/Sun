...
const API_KEY = "tiendat"; // Thay đổi key này bằng key của bạn
...
fastify.addHook("onRequest", async (request, reply) => {
  if (request.url.startsWith("/api/sunwin") || request.url.startsWith("/api/history-json")) {
    ...
    if (!urlKey || urlKey !== API_KEY) {
      return reply.code(403).send({ error: "mày ngu địtmẹ mày đéo có key mà đòi đớp à có tiền hì ib bố @tiendataox" });
    }
  }
});
...
const HIGH_CONFIDENCE_THRESHOLD = 0.68; // Ngưỡng du_doan rất tự tin (slightly increased)
const MODERATE_CONFIDENCE_THRESHOLD = 0.55; // Ngưỡng du_doan trung bình (slightly increased)
...
async function broadcastPrediction() {
    ...
    db.all(`SELECT sid, d1, d2, d3, total, result, timestamp FROM sessions ORDER BY sid DESC LIMIT 1000`, async (err, rows) => {
        ...
        if (totalEffectiveWeight > 0) {
            ...
            if (taiConfidence > xiuConfidence * 1.05 && taiConfidence >= 0.48) {
                finalPrediction = "Tài";
            ...
            } else if (xiuConfidence > taiConfidence * 1.05 && xiuConfidence >= 0.48) {
                finalPrediction = "Xỉu";
                ...
                if (lastSession) {
                    ...
                    contributingLogics = ["Fallback: Theo Phien"];
                } else {
        ...
        const MAX_OVERALL_CONFIDENCE_DISPLAY = 97;
        ...
        const predictionMessage = {
                "Phien": lastSessionIdDisplay,
                "ket_qua": lastSessionResultDisplay,
                "Xuc_xac_1": lastSessionDice ? lastSessionDice[0] : null,
                "Xuc_xac_2": lastSessionDice ? lastSessionDice[1] : null,
                "Xuc_xac_3": lastSessionDice ? lastSessionDice[2] : null,
                "phien_hien_tai": nextSessionId,
                "du_doan": finalPrediction,
                "do_tin_cay": `${Math.round(overallConfidence)}`,
...
function predictLogic4(history) {
  if (history.length < 25) return null;  // Giảm từ 30
  ...
  for (const len of patternLengths) {
    ...
    // Tăng tốc độ tìm kiếm pattern
    for (let i = len; i < Math.min(history.length - 1, 150); i++) { // Giảm từ 200
...
function predictLogic7(history) {
  const TREND_STREAK_LENGTH_MIN = 3;  // Giảm từ 4
  const TREND_STREAK_LENGTH_MAX = 6;  // Giảm từ 7
  
  if (history.length < TREND_STREAK_LENGTH_MIN) return null;

  const volatility = calculateStdDev(history.slice(0, Math.min(20, history.length)).map(s => s.total)); // Giảm từ 25

  // Điều chỉnh streak length dựa trên volatility
  const effectiveStreakLength = (volatility < 1.7) ? TREND_STREAK_LENGTH_MAX : 
                               (volatility < 2.0) ? TREND_STREAK_LENGTH_MAX - 1 : 
                               TREND_STREAK_LENGTH_MIN;
  ...
  // Kiểm tra cả continuation và reversal
  if (recentResults.every(r => r === "Tài")) {
    // Tìm historical continuation sau streak
    ...
    for (let i = effectiveStreakLength; i < Math.min(history.length - 1, 100); i++) {
      ...
      if (potentialStreak.every(s => s.result === "Tài")) {
        if (history[i - 1].result === "Tài") continuationCount++;
        else reversalCount++;
      }
    }
    
    if (continuationCount > reversalCount * 1.5) {
      return "Tài";
    } else if (reversalCount > continuationCount * 1.5) {
      return "Xỉu";
    }
  }
  ...
  if (recentResults.every(r => r === "Xỉu")) {
    ...
    for (let i = effectiveStreakLength; i < Math.min(history.length - 1, 100); i++) {
      ...
      if (potentialStreak.every(s => s.result === "Xỉu")) {
        if (history[i - 1].result === "Xỉu") continuationCount++;
        else reversalCount++;
      }
    }
...
function predictLogic9(history) {
  if (history.length < 15) return null;  // Giảm từ 20
  ...
  if (currentConsecutiveCount < 2) return null;  // Giảm từ 3

  let totalReversals = 0;
  let totalContinuations = 0;
  
  // Tìm kiếm trong lịch sử gần đây
  const searchWindow = Math.min(history.length, 80);  // Giảm từ 120
  ...
  if (totalReversals + totalContinuations > 2) {  // Giảm từ 3
    if (totalReversals > totalContinuations * 1.2) {  // Giảm từ 1.3
      return mostRecentResult === "Tài" ? "Xỉu" : "Tài";
    } else if (totalContinuations > totalReversals * 1.2) {
...
async function predictLogic20(history, logicPerformance, cauLogData) { // Added cauLogData as a parameter
  ...
  const signals = [
    { logic: 'logic1', baseWeight: 0.7 },    // Giảm nhẹ
    { logic: 'logic2', baseWeight: 0.8 },    // Tăng nhẹ
    { logic: 'logic3', baseWeight: 0.8 },    // Giảm từ 0.9
    { logic: 'logic4', baseWeight: 1.4 },    // Tăng từ 1.2 (pattern quan trọng)
    { logic: 'logic5', baseWeight: 0.7 },    // Tăng từ 0.6
    { logic: 'logic6', baseWeight: 0.9 },    // Tăng từ 0.8
    { logic: 'logic7', baseWeight: 1.1 },    // Tăng từ 1.0 (streak quan trọng)
    { logic: 'logic8', baseWeight: 0.8 },    // Tăng từ 0.7
    { logic: 'logic9', baseWeight: 1.2 },    // Tăng từ 1.1 (consecutive quan trọng)
    { logic: 'logic10', baseWeight: 1.0 },   // Tăng từ 0.9
    { logic: 'logic11', baseWeight: 1.4 },   // Tăng từ 1.3 (reversal pattern)
    { logic: 'logic12', baseWeight: 0.8 },   // Tăng từ 0.7
    { logic: 'logic13', baseWeight: 1.3 },   // Tăng từ 1.2
    { logic: 'logic14', baseWeight: 0.9 },   // Tăng từ 0.8
    { logic: 'logic15', baseWeight: 0.7 },   // Tăng từ 0.6
    { logic: 'logic16', baseWeight: 0.8 },   // Tăng từ 0.7
    { logic: 'logic17', baseWeight: 1.0 },   // Tăng từ 0.9
    { logic: 'logic18', baseWeight: 1.4 },   // Tăng từ 1.3 (dice parity quan trọng)
    { logic: 'logic19', baseWeight: 1.0 },   // Tăng từ 0.9
    { logic: 'logic21', baseWeight: 1.6 },   // Tăng từ 1.5 (multi-window quan trọng)
    { logic: 'logic22', baseWeight: 1.9 },   // Tăng từ 1.8 (super-powered cau analysis)
    { logic: 'logic23', baseWeight: 1.1 },   // Tăng từ 1.0
    { logic: 'logic24', baseWeight: 1.2 }    // Tăng từ 1.1
];
...
fastify.get("/api/sunwin", async (request, reply) => {
  return new Promise((resolve, reject) => {
    db.all(`SELECT sid, d1, d2, d3, total, result, timestamp FROM sessions ORDER BY sid DESC LIMIT 1000`, async (err, rows) => {
      ...
      reply.type('application/json').send({
        "Phien": lastSessionIdDisplay,
        "Xuc_xac_1": lastSessionDice[0],
        "Xuc_xac_2": lastSessionDice[1],
        "Xuc_xac_3": lastSessionDice[2],
        "Ket_qua": lastSessionResultDisplay,
        "phien_hien_tai": nextSessionId,
        "du_doan": finalPrediction,
        "do_tin_cay": `${Math.round(overallConfidence)}`,
...
const start = async () => {
  try {
    ...
    console.log(`WebSocket API (use ws:// with public IP): ws://YOUR_PUBLIC_IP:${PORT}/api/sunwin/taixiu/ws?id=mrtinhios&key=${API_KEY}`);
  } catch (err) {
