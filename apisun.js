...
const API_KEY = "tiendat";

// Middleware for HTTP API
fastify.addHook("onRequest", async (request, reply) => {
  if (request.url.startsWith("/api/sunwin") || request.url.startsWith("/api/history-json") || request.url.startsWith("/api/his") || request.url.startsWith("/api/analysis")) {
...
function logCauPattern(patternData) {
  fs.appendFile(cauLogFilePath, JSON.stringify(patternData) + '\n', (err) => {
    if (err) console.error("Lỗi ghi log cầu:", err);
  });
}
...
function updateLogicPerformance(logicName, predicted, actual) {
  ...
  let decay = 0.95;
  if (perf.total > 0 && perf.accuracy < 0.6) decay = 0.85;
  else if (perf.total > 0 && perf.accuracy > 0.8) decay = 0.98;
  perf.correct *= decay;
  perf.total *= decay;
  perf.total++;
  let wasCorrect = 0;
...
function sendHistoryToClient(ws) {
  db.all(`SELECT sid, prediction, actual, status, confidence, cau_hien_tai, ly_do, timestamp FROM predictions ORDER BY sid DESC LIMIT 500`, (err, rows) => {
    if (err) return;
...
async function updateLogicWeights() {
  db.all(`SELECT prediction, actual FROM predictions WHERE actual IS NOT NULL ORDER BY sid DESC LIMIT 100`, (err, rows) => {
    if (err || rows.length < 20) return;
    ...
    try {
      ...
      if (Array.isArray(json) && json[1]?.htr) {
        ...
        for (const newItem of incomingResults) {
          ...
          const row = await new Promise((resolve) => db.get(`SELECT sid FROM sessions WHERE sid = ?`, [newItem.sid], (err, row) => resolve(row)));
          if (!row) {
            const result = total <= 10 ? "Xỉu" : "Tài";
            const timestamp = Date.now();
            await new Promise((resolve) => db.run(`INSERT INTO sessions (sid, d1, d2, d3, total, result, timestamp) VALUES (?,?,?,?,?,?,?)`,
              [newItem.sid, newItem.d1, newItem.d2, newItem.d3, total, result, timestamp], resolve));
            db.run(`UPDATE predictions SET actual = ?, status = ? WHERE sid = ?`, [result, result === 'Tài' ? 'win' : 'lose', newItem.sid]);
            db.all(`SELECT sid, d1, d2, d3, total, result FROM sessions ORDER BY sid DESC LIMIT 50`, (histErr, recent) => {
              if (!histErr && recent.length > 5) {
...
async function broadcastPrediction() {
  db.all(`SELECT sid, d1, d2, d3, total, result, timestamp FROM sessions ORDER BY sid DESC LIMIT 1000`, async (err, rows) => {
    if (err) {
      console.error("Lỗi truy vấn DB broadcast:", err.message);
      return;
    }
    ...
    if (overallConfidence !== "N/A") {
      overallConfidence = Math.min(parseFloat(overallConfidence), 97).toFixed(2);
    }
    ...
    const timestamp = Date.now();
    db.run(`INSERT INTO predictions (sid, prediction, confidence, cau_hien_tai, ly_do, timestamp) VALUES (?, ?, ?, ?, ?, ?)`,
...
fastify.get("/api/sunwin", async (request, reply) => {
  return new Promise((resolve, reject) => {
    db.all(`SELECT sid, d1, d2, d3, total, result, timestamp FROM sessions ORDER BY sid DESC LIMIT 1000`, async (err, rows) => {
      ...
      const history = rows.filter(i => i.d1 && i.d2 && i.d3);
      if (history.length < 5) {
        reply.send({ error: "Chưa đủ dữ liệu" });
        return resolve();
      }
      const lastSession = history[0];
      const nextSessionId = lastSession.sid + 1;
      // Tính toán tương tự broadcast nhưng không lưu DB
      // (có thể gọi lại logic tính toán, nhưng để đơn giản, ta gọi broadcast và lấy kết quả từ cache? Ở đây tạm thời trả về thông báo)
      reply.send({ message: "Dùng WebSocket để nhận dữ liệu realtime" });
      resolve();
    });
  });
});
...
fastify.get("/api/analysis", async (request, reply) => {
  return new Promise((resolve, reject) => {
    db.all(`SELECT sid, d1, d2, d3, total, result, timestamp FROM sessions ORDER BY sid DESC LIMIT 100`, (err, rows) => {
      if (err) {
        reply.status(500).send({ error: err.message });
        return reject();
      }
...
const start = async () => {
  ...
  // Khởi tạo logic performance từ log (nếu có) - có thể gọi initializeLogicPerformanceFromLog nếu muốn
